...
class JOOOIDModelArticles extends ContentModelArticles
{
	...
	{
		$joooid_db_escape = JOOOID_DB_ESCAPE;
		//TODO check in tutti i file degli escape e le cose scritte sulla compatibilita' di joomla 3
		$db = $this->getDbo();
		...
		$query->select(
			$this->getState(
				...
				', a.introtext, a.fulltext, a.metakey, a.metadesc, a.metadata'
			)
		);
		...
		$search = $this->getState('filter.search');
			if (!empty($search)) {
				if (stripos($search, 'id:') === 0) {
					...
					$search = $db->Quote('%'.$db->$joooid_db_escape(substr($search, 7), true).'%');
					$query->where('(ua.name LIKE '.$search.' OR ua.username LIKE '.$search.')');
				}
				else {
					$search = $db->Quote('%'.$db->$joooid_db_escape($search, true).'%');
		...
		//print_r($orderCol);print_r($orderDirn);die;
		$query->order($db->$joooid_db_escape($orderCol.' '.$orderDirn));
		//$query->setLimit(0,1);
		return $query;

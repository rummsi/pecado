...
class plgXmlrpcJoooid extends JPlugin
{
	...
	public function onGetWebServices()
	{
		...
		return array
			(
			 ...
			 'joooid.getAccessLevelList' => array( 'function' => array($this, 'getAccessLevelList'), 'signature' => null ),
			 ...
			 'joooid.addUsergroup' => array( 'function' => array($this, 'addUsergroup'), 'signature' => null ),
			 ...
			 'joooid.setConfig' => array( 'function' => array($this, 'setConfig'), 'signature' => null ),
	...
	public function widget ($key,$username,$password,$param=null){
		global $xmlrpcArray;
		global $xmlrpcStruct;
		...
		JRequest::setVar('order_key','created');
		$modelArticles->setState('list.ordering','created');
		JRequest::setVar('order_filter','desc');
		$modelArticles->setState('list.direction','desc');

		$temp = $modelArticles->getItems();
		...
		// Get the number of frontend sessions number in last 10 minutes
		$lastTenMinutes = mktime (date("G"),date("i")-10,date("s"),date("m")  ,date("d"),date("Y"));
		$query->clear();
		$query->select('COUNT(session_id)');
		$query->from('#__session');
		$query->where('guest = 1 AND client_id = 0 AND time>'.$lastTenMinutes.'');
		$db->setQuery($query);
	...
	public function setConfig ($key,$username,$password,$config){
			...
			foreach ($tmp as $row){
				...
				if(($row->menutype==$menutype)||true){
					$xmlArray = array(
							...
							'type'			=> new xmlrpcval( $row->type, 'string' ),
	...
	public function addUsergroup ($key,$username,$password,$parent_id,$title,$id){
	...
	public function newMenuItem ($key,$username,$password,$title,$alias,$note,$menutype,$component_id,$link,$published,$access,$parent_id,$img,$ordering,$params,$language,$type,$id){
		...
		$data['access'] = $access;
		$data['apublished'] = 1;
		if(intval($parent_id)!=0 && intval($parent_id)!=1){
			$data['parent_id'] = $parent_id;
		}
		else{
			$data['parent_id'] = 1;
	...
	public function getAccessLevelList($key,$username,$password){
		$user = $this->authenticateUser($username, $password);
		...
		foreach($items as $row){
			...
			$structarray[] = new xmlrpcval(
					array(
						...
						'rules' => new xmlrpcval($row->rules, 'string'),
					     ),'struct');
		}
		if(empty($structarray)){
			return $this->response(JText::_('PLG_JOOOID_ELEMENTS_WAS_NOT_FOUND'));
	...
	protected function buildCategory($title,$alias,$description,$parent_id,$published,$access,$created_user_id,$language,$metadesc,$metakey,$metadata,$id)
	{
	...
	public function blogger_newCategory()
	{
		...
		return $this->mw_newCategory($username, $password, $title,$alias,$description,$parent_id, $published, $access, $created_user_id,$language,$metadesc,$metakey,$metadata,$id);
	}
	...
	public function mw_newCategory()
	{
		...
		$data  = $this->buildCategory($title,$alias,$description,$parent_id,$published,$access,$created_user_id,$language,$metadesc,$metakey,$metadata,$id);
		$model = $this->getModel('Category');
		...
		foreach($categories as $row){
			...
			$structarray[] = new xmlrpcval(
					array(
						...
						'language' => new xmlrpcval($row->language, 'string'),
						'author_name' => new xmlrpcval($row->author_name, 'string'),
						'created_user_id' => new xmlrpcval($row->created_user_id, 'string'),
						'metadesc' => new xmlrpcval($row->metadesc, 'string'),
						'metakey' => new xmlrpcval($row->metakey, 'string'),
						'metadata' => new xmlrpcval($row->metadata, 'string'),
						'published' => new xmlrpcval($row->published, 'string')
					     ),'struct');
		}
		if(empty($structarray)){
	...
	public function blogger_newPost()
	{
		...
		if(is_array($args[3])){
			$username		= strval( $args[1] );
			$password		= strval( $args[2] );
			$blogid			= isset($args[3]['catid'])?(int)($args[3]['catid']):0;
			$title			= isset($args[3]['title'])?$args[3]['title']:'';
			$alias			= isset($args[3]['alias'])?$args[3]['alias']:'';
			$intro			= isset($args[3]['introtext'])?$args[3]['introtext']:'';
			$full			= isset($args[3]['fulltext'])?$args[3]['fulltext']:'';
			$publish		= isset($args[3]['state'])?(int)$args[3]['state']:'';
			$access			= isset($args[3]['access'])?(int)$args[3]['access']:'';
			$front			= isset($args[3]['featured'])?(int)$args[3]['featured']:'';
			$published_up		= isset($args[3]['publish_up'])?$args[3]['publish_up']:'';
			$published_down		= isset($args[3]['publish_down'])?$args[3]['publish_down']:'';
			$created		= isset($args[3]['created'])?$args[3]['created']:'';
			$now			= isset($args[3]['current_date'])?$args[3]['current_date']:'';
			$language 		= isset($args[3]['language'])?$args[3]['language']:'';
			$created_by 		= isset($args[3]['created_by'])?$args[3]['created_by']:null;
			$created_by_alias	= isset($args[3]['created_by_alias'])?$args[3]['created_by_alias']:null;
			$metakey 		= isset($args[3]['metakey'])?$args[3]['metakey']:null;
			$metadesc		= isset($args[3]['metadesc'])?$args[3]['metadesc']:null;
			$metadata		= isset($args[3]['metadata'])?$args[3]['metadata']:null;
		...
		if ($created !='0'){
			...
			$ora = date ("H",$created)-$difference;
			$minuti = date ("i",$created);
			$secondi = date ("s",$created);
			$anno = date ("Y",$created);
			$mese = date ("m",$created);
			$giorno = date ("d",$created);

			$created = date( "Y-m-d H:i:s",mktime($ora,$minuti,$secondi,$mese,$giorno,$anno ));
		}

		if ($published_down !='0'){


			$published_down = strtotime($published_down);
			$ora = date ("H",$published_down)-$difference;
			$minuti = date ("i",$published_down);
			$secondi = date ("s",$published_down);
			$anno = date ("Y",$published_down);
			$mese = date ("m",$published_down);
			$giorno = date ("d",$published_down);

			$published_down = date( "Y-m-d H:i:s",mktime($ora,$minuti,$secondi,$mese,$giorno,$anno ));
		}
		...
		return $this->mw_newPost($blogid, $username, $password, $title, $alias, $intro, $full, $publish, $access,$front, $published_up, $difference, $language,$published_down,$created,$created_by,$created_by_alias,$metakey,$metadesc,$metadata);
	}
	...
	public function blogger_editPost()
	{
		...
		if(is_array($args[3])){
			...
			$username	= strval( $args[1] );
			$password	= strval( $args[2] );
			$postid		= isset($args[3]['id'])?(int)$args[3]['id']:0;
			$catId 		= isset($args[3]['catid'])?(int)($args[3]['catid']):0;
			$title		= isset($args[3]['title'])?$args[3]['title']:'';
			$alias		= isset($args[3]['alias'])?$args[3]['alias']:'';
			$intro		= isset($args[3]['introtext'])?$args[3]['introtext']:'';
			$full		= isset($args[3]['fulltext'])?$args[3]['fulltext']:'';
			$state		= isset($args[3]['state'])?(int)$args[3]['state']:'';
			$access		= isset($args[3]['access'])?(int)$args[3]['access']:'';
			$front		= isset($args[3]['featured'])?(int)$args[3]['featured']:'';
			$published_up	= isset($args[3]['publish_up'])?$args[3]['publish_up']:'';
			$published_down	= isset($args[3]['publish_down'])?$args[3]['publish_down']:'';
		...
		return $this->mw_editPost($postid, $username, $password, $title, $alias, $intro, $full, $state, $access, $front, $published_up, $catId, $difference,$language,$published_down,$created,$created_by,$created_by_alias,$metakey,$metadesc,$metadata);
	}
	...
	public function mw_newPost()
	{
		...
		$language 		= $args[12];
		$published_down		= $args[13];
		$created 		= $args[14];
		$created_by 		= $args[15];
		$created_by_alias 	= $args[16];
		$metakey 		= $args[17];
		$metadesc		= $args[18];
		$metadata 		= $args[19];
		...
		$data  = $this->buildArticle($title, $alias, $intro, $full, $publish, $access, $front,$language,$published_up,$published_down,$created,$created_by, $created_by_alias,$metakey,$metadesc,$metadata);
		$data['featured'] = $front;
	...
	public function mw_editPost()
	{
		...
		$catid	 	= $args[11];
		$language 	= $args[13];
		$published_down	= $args[14];
		$created 	= $args[15];
		$created_by 	= $args[16];
		$created_by_alias= $args[17];
		$metakey 	= $args[18];
		$metadesc 	= $args[19];
		$metadata 	= $args[20];
		...
		$data  = $this->buildArticle($title, $alias, $intro, $full, $state, $access, $front, $language, $published_up,$published_down,$created,$created_by,$created_by_alias,$metakey,$metadesc,$metadata);
		$data['id'] = $postid;
		$data['featured'] = $front;
	...
	public function mw_getRecentPosts()
	{
		...
		/* -- begin -- */
		$blogid = (int)$blogid;
		if($blogid > 0 && !isset($filters['category_id'])){
		...
		if(count($temp)){
...

			{
				...
				$res = $this->buildStruct($row, false);
	...
	public function mw_newMediaObject()
	{
		...
		$images_path = str_replace('/', JOOOID_DS, JPATH_ROOT. JOOOID_DS. $params->get('image_path', 'images'));
		$file_path = str_replace('/', JOOOID_DS, JPATH_ROOT. JOOOID_DS. $params->get('file_path', 'images'));
		...
		if(!empty($dir)){
			...
			if(!JFile::exists($destination. JOOOID_DS. 'index.html')){
				$html = '<html><body></body></html>';
				JFile::write($destination. JOOOID_DS. 'index.html', $html);
			}
		...
		$url = JURI::root(true). str_replace(array(JPATH_ROOT, JOOOID_DS), array('', '/'), $destination. $file_name);
	...
	protected function buildStruct($row, $mt=false)
	{
		...
		if($mt){
			...
			$xmlArray = array(
					...
					'author_name'		=> new xmlrpcval( isset($row->author_name)?$row->author_name:'', 'string' ),
					'created_by'		=> new xmlrpcval( $row->created_by , 'string' ),
					'created_by_alias'	=> new xmlrpcval( $row->created_by_alias , 'string' ),
					'postid'		=> new xmlrpcval( $row->id, 'string' ),
					'description'		=> new xmlrpcval( $row->introtext, 'string' ),
					'title'			=> new xmlrpcval( $row->title, 'string' ),
					'alias' 		=> new xmlrpcval( $row->alias, 'string'),
					'catid' 		=> new xmlrpcval( $row->catid, 'string'),
					'categories'		=> new xmlrpcval( array( new xmlrpcval((isset($row->category_title)?$row->category_title:''), 'string') ) , 'array' ),
					'link'			=> new xmlrpcval( $link, 'string' ),
					'text_more'		=> new xmlrpcval( $row->fulltext, 'string' ),
					'metadesc'		=> new xmlrpcval( $row->metadesc, 'string' ),
					'metakey'		=> new xmlrpcval( $row->metakey, 'string' ),
					'metadata'		=> new xmlrpcval( htmlspecialchars_decode($row->metadata), 'string' ),
					'language'		=> new xmlrpcval( $row->language, 'string' ),
					'permissions'		=> new xmlrpcval( (isset($row->permissions)?$row->permissions:''), 'string' ),
					'state'			=> new xmlrpcval( $row->state, 'string' ),
					'publish_up'		=> new xmlrpcval( $row->publish_up, 'string' ),
					'publish_down'		=> new xmlrpcval( $row->publish_down, 'string' ),
					'access'		=> new xmlrpcval( $row->access, 'string' ),
